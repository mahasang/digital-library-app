# Mobile App — Home Screen Redesign (Play Books style)

## ขอบเขต
- `app/(tabs)/index.tsx` — full replace
- `app/(tabs)/account.tsx` — เพิ่ม 🔔 ที่ header + ย้าย notifications
- ห้ามแตะไฟล์อื่น
...
### index.tsx
1. **Header** — แทน logoBox + notifBtn ด้วย avatar user (ถ้า login) หรือ icon library (ถ้าไม่ login)
2. **Card ใหญ่ขึ้น** — `H_CARD_WIDTH: 140`, `H_COVER_HEIGHT: 196` (ratio 1:1.4)
3. **Rating แสดงเป็นตัวเลข** — `4.6 ★` แทน 5 ดาว placeholder
4. **ลบ 🔔** จาก header (ย้ายไป account.tsx)

### account.tsx
1. **เพิ่ม 🔔** ที่ header ขวาสุด navigate ไป `/notifications`
...
เพิ่ม 🔔 ที่ header — แก้เฉพาะ **2 จุด**:

### จุดที่ 1: header ของ logged-in view
```tsx
// เดิม
<View style={styles.header}>
...
### จุดที่ 2: header ของ guest view
```tsx
// เดิม
<View style={styles.header}>
...
### แก้ header style ให้รองรับ row layout:
```ts
header: {
  flexDirection: 'row',           // เพิ่ม
  justifyContent: 'space-between', // เพิ่ม
...
```
@app/(tabs)/index.tsx @app/(tabs)/account.tsx

แก้ตาม prompt file นี้:
1. index.tsx: full replace — card ใหญ่ขึ้น (140px), rating เป็นตัวเลข,
   avatar user ที่ header แทน 🔔, โหลด ratings จริงจาก RPC
2. account.tsx: เพิ่ม 🔔 ที่ header ทั้ง logged-in และ guest view
ห้ามแตะไฟล์อื่น
```
...
2. หน้าแรก — card กว้าง 140px สูง 196px
3. login แล้ว → avatar วงกลมขวาบน กดไปหน้า account
4. ไม่ login → icon 👤 ขวาบน กดไปหน้า login
5. rating แสดง `4.6 ★` ถ้ามีคะแนน, ไม่แสดงถ้าไม่มี
6. หน้า ຕັ້ງຄ່າ — icon 🔔 ขวาบน navigate ไป notifications
