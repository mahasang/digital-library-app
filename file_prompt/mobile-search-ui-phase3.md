# Mobile App — Phase 3: Search UI Redesign

## เป้าหมาย
ปรับปรุง UI หน้าค้นหา 2 ไฟล์:
1. `app/search.tsx` — grid 3 คอลัมน์ + filter radio chips แบบใหม่
2. `app/(tabs)/research.tsx` — grid 3 คอลัมน์ให้ consistent กัน

## ห้ามแตะ
- `lib/research.ts` — logic และ query ไม่เปลี่ยน
- `lib/supabase.ts`
- ไฟล์อื่นทั้งหมดที่ไม่ได้ระบุ

---

## ไฟล์ที่ 1: `app/search.tsx`
...
## ไฟล์ที่ 2: `app/(tabs)/research.tsx`

**เปลี่ยนเฉพาะส่วน grid ให้เป็น 3 คอลัมน์ เหมือนกัน:**
...
```
@app/search.tsx @app/(tabs)/research.tsx

ปรับ UI ตาม prompt file นี้:
- search.tsx: grid 3 คอลัมน์ + radio chips filter
- research.tsx: grid 3 คอลัมน์ consistent

ห้ามแตะ lib/research.ts และ lib/supabase.ts
```

## ตรวจสอบหลังรัน

1. เปิด Expo Go บนมือถือ
2. ไปหน้า ຊັ້ນຫນັງສື — ควรเห็น 3 คอลัมน์
3. กดค้นหา — ควรเห็น 3 คอลัมน์ + radio chips ใต้ header
4. กด radio chip เปลี่ยนหมวดหมู่ — ผลลัพธ์ควรกรองตาม

## ความเสี่ยง / สิ่งที่ต้องระวัง

| รายการ | ความเสี่ยง | การป้องกัน |
|--------|-----------|------------|
| CARD_WIDTH คำนวณจาก Dimensions | หน้าจอพับได้หรือ tablet อาจกว้างเกิน | ใช้กับ Samsung S24 Ultra ปกติ ✅ |
| radio chips แสดงแค่ 4 หมวดแรก | ถ้ามีหมวดเยอะกว่านี้จะไม่แสดง | tab bar ด้านบนยังแสดงครบ ✅ |
| `lib/research.ts` ไม่เปลี่ยน | bug เดิมถ้ามียังคงอยู่ | แก้ใน phase แยกต่างหาก |
